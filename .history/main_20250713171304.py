...
if __name__ == "__main__":
    ...
    job_path = prompt_filepath("Enter path to job description (.txt or .pdf)", "test_files/job1.txt")
    resume_path = prompt_filepath("Enter path to resume (.txt or .pdf)", "test_files/resume1.txt")
    ...
    # Prompt user for extraction mode
    ...
    # Sort matched and missing by frequency (then alphabetically)
    ...
    # 4. Print Results (with frequency table)
    print(f"\n=== RESULTS ===")
    print(f"Match Percent: {match_percent:.1f}%\n")

    print(f"Matched Keywords ({len(sorted_matched)}):")
    print(f"{'Keyword':<20} {'Frequency in JD':>16}")
    print("-" * 36)
    for word in sorted_matched:
        print(f"{word:<20} {job_word_counts[word]:>16}")

    print(f"\nMissing Keywords ({len(sorted_missing)}):")
    print(f"{'Keyword':<20} {'Frequency in JD':>16}")
    print("-" * 36)
    ...
    if save_choice == "txt":
        ...
        save_results_txt(filename, match_percent, sorted_matched, sorted_missing, job_word_counts)
        print(f"Results saved to {filename}")
    elif save_choice == "csv":
        filename = input("Enter filename (e.g., output.csv): ").strip()
        if not filename.startswith("output/"):
            filename = os.path.join("output", filename)
        save_results_csv(filename, match_percent, sorted_matched, sorted_missing, job_word_counts)
        print(f"Results saved to {filename}")
    else:
