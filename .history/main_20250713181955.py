...
# ----- At the top of your file, your default sample resumes -----
DEFAULT_RESUMES = [
...
if __name__ == "__main__":
    ...
    # Prompt for multiple resume files (comma-separated)
    resume_paths_input = input(
        "Enter path(s) to resume file(s) (.txt or .pdf), separated by commas\n"
        "[Press Enter to use test_files/resume1.txt]: "
    ).strip()
    if not resume_paths_input:
        resume_paths = ["test_files/resume1.txt"]
    else:
        # Split, strip whitespace, and filter out empty
        resume_paths = [p.strip() for p in resume_paths_input.split(",") if p.strip()]
